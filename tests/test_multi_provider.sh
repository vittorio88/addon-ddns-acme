...
cat > "$CONFIG_PATH" <<'JSON'
{
  ...
  "log_level": "info",
  "dns_accounts": [
    {"provider": "dynu", "token": "dynu-token", "domains": ["dynu1.example.com", "dynu2.example.com"]},
...
expected=$'dynu|token=dynu-token|domain=dynu1.example.com|ipv4=203.0.113.10|ipv6=\ndynu|token=dynu-token|domain=dynu2.example.com|ipv4=203.0.113.10|ipv6=\nduckdns|token=duck-token|domain=duck.example.org|ipv4=203.0.113.10|ipv6='
actual=$(cat "$CALLS")
...
echo "PASS multi-provider DDNS dispatch"
